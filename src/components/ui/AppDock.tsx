      ...
      <nav className="flex items-center gap-2 rounded-full border border-border dark:border-white/10 bg-white/80 dark:bg-black/40 px-4 py-3 backdrop-blur-2xl shadow-[0_8px_30px_-8px_rgba(232,69,107,0.15),0_2px_8px_-2px_rgba(0,0,0,0.05)] dark:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.1)]">
        ...
        {isMounted ? (
          <button 
            ...
            className="flex items-center justify-center h-12 w-12 rounded-full text-gold hover:bg-black/5 dark:hover:bg-white/5 transition-colors group mr-1 sm:mr-2"
            title="Toggle Theme"
          >
        ...
        {tabs.slice(0, 2).map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`outline-none group relative flex h-12 w-12 sm:w-16 flex-col items-center justify-center rounded-full transition-all duration-300 hover:bg-black/5 dark:hover:bg-white/5 focus-visible:bg-black/10 dark:focus-visible:bg-white/10 ${isActive ? "text-primary dark:text-blush" : "text-muted-foreground hover:text-foreground"}`}
            >
