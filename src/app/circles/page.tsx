    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-8 py-8 relative">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
          ...
          <div className="p-6 rounded-2xl border border-dashed border-border/50 bg-card/30 text-center transition-all cursor-pointer hover:-translate-y-0.5 hover:shadow-md hover:border-primary/30 active:translate-y-0 active:shadow-sm active:border-primary/40">
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <button onClick={() => setShowJoinModal(true)} className="inline-flex items-center gap-1 px-4 py-2 rounded-full glass-btn text-[11px] font-semibold touch-target"><LogIn className="h-3.5 w-3.5" /> Join</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="p-5 rounded-2xl border border-border bg-card/40 backdrop-blur-sm mb-8">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="w-10 h-10 rounded-full border border-dashed border-border bg-card/40 flex items-center justify-center text-muted-foreground hover:text-foreground hover:border-primary/40 hover:bg-primary/10 active:text-foreground active:border-primary/50 active:bg-primary/15 transition-all shrink-0 ml-2 touch-target"
          >
                ...
                {searchResults.map((u: any) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between px-3 py-2 rounded-xl transition-colors ${status === 'none' ? 'cursor-pointer hover:bg-card/40 active:bg-card/60' : ''}`}
                    >
                      <Link href={`/users/${u.id}`} onClick={(e) => e.stopPropagation()} className="text-sm text-foreground underline decoration-dotted decoration-muted-foreground/30 hover:text-primary active:text-primary/80 transition-colors flex items-center gap-2 min-w-0 flex-1">
                        {u.avatar_url ? (
                          <img src={u.avatar_url} alt="" loading="lazy" className="w-6 h-6 rounded-full object-cover shrink-0" />
                        ) : (
      ...
      {!requestsLoading && (incomingRequests.length > 0 || outgoingRequests.length > 0) && (
        <div className="rounded-2xl border border-border bg-card/40 backdrop-blur-sm overflow-hidden">
          <button onClick={() => setRequestsOpen(!requestsOpen)} className="w-full flex items-center justify-between p-5 hover:bg-card/20 active:bg-card/30 transition-colors">
            ...
            <div className="px-5 pb-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      incomingRequests.map(req => {
                        ...
                        return (
                          <div key={req.id} className="flex items-center justify-between px-3 py-2 rounded-xl border border-border/60 bg-card/50">
                            <Link href={`/users/${sender?.id}`} className="text-sm text-foreground underline decoration-dotted decoration-muted-foreground/30 flex items-center gap-2 hover:text-primary active:text-primary/80 transition-colors min-w-0 flex-1">
                ...
                <div className="p-4 rounded-xl border border-warning/15 bg-warning/[0.03]">
                  <p className="text-[10px] uppercase tracking-wider text-warning font-bold mb-3 flex items-center gap-1.5">
                      ...
                      outgoingRequests.map(req => {
                        ...
                        return (
                          <div key={req.id} className="flex items-center justify-between px-3 py-2 rounded-xl border border-dashed border-warning/20 bg-card/40">
                            <Link href={`/users/${receiver?.id}`} className="text-sm text-foreground/70 flex items-center gap-2 hover:text-foreground active:text-foreground transition-colors min-w-0 flex-1">
      ...
      <Modal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} title="Create New Bond">
        <form onSubmit={handleCreate} className="space-y-4 mt-2">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {emojiOptions.map(e => (
                <button key={e} type="button" onClick={() => setNewEmoji(e)} className={`w-10 h-10 rounded-xl text-lg flex items-center justify-center transition-all ${newEmoji === e ? 'bg-primary/20 border-2 border-primary scale-110' : 'bg-muted/30 border border-border hover:bg-muted/50 active:bg-muted/70'}`}>{e}</button>
              ))}
            </div>
          </div>
          <div>
